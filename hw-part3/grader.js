...
var CHECKSURL_DEFAULT = "http://dry-hamlet-6752.herokuapp.com";
...
var cheerioHtmlUrl = function(htmlUrl,checksfile) {
        rest.get(htmlUrl+'').on('complete', function(result) {
          if (result instanceof Error) {
            console.log("Failed to connect to "+htmlUrl);
            //this.retry(5000); // try again after 5 sec
            process.exit(1);
          } else {
              doCheck(result,checksfile);
          }
        });
};
...
var doCheck = function(result, checksfile){
...
if(require.main == module) {
	...
	.option('-u, --url <url_path>', 'url to index.html', clone(assertUrlExists) /* , CHECKSURL_DEFAULT */)
        .parse(process.argv);
